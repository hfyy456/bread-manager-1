...
const RecipeCreateDialog = ({
  ...
  const subRecipeSource =
    recipeType === "dough" ? doughRecipes : fillingRecipes;
  const subRecipePayloadKey =
    recipeType === "dough" ? "preFerments" : "subFillings";
  ...
  const handleSubmit = async () => {
    ...
    if (recipe.subRecipes && recipe.subRecipes.length > 0) {
      payload[subRecipePayloadKey] = recipe.subRecipes
        ...
        .map((sub) => {
          // Find the recipe from the source data by ID or name
          const sourceRecipe =
            (subRecipeSource || []).find((r) => r.id === sub.recipeId) ||
            (subRecipeSource || []).find((r) => r.name === sub.name);

          return {
            id: sourceRecipe ? sourceRecipe.id : sub.recipeId,
            name: sub.name || (sourceRecipe ? sourceRecipe.name : sub.recipeId),
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      ...
      <DialogContent>
          ...
          >
            <Grid item xs={12} sm={6}>
              <Autocomplete
                ...
                value={
                  ingredients.find((i) => i.name === ing.ingredientId) || null
                }
                onChange={(_, newValue) => {
                  handleDynamicChange(
                    "ingredients",
                ...
                renderInput={(params) => (
                  <TextField {...params} label="原料" required />
                )}
              />
            </Grid>
          ...
          >
            <Grid item xs={12} sm={8}>
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField {...params} label="配方" required />
                )}
              />
            </Grid>
            <Grid item xs={12} sm={3}>
            ...
            <Grid item xs={12} sm={1}>
              <IconButton onClick={() => removeListItem("subRecipes", index)}>
                <RemoveCircleOutlineIcon />
              </IconButton>
            </Grid>
          </Grid>
        ...
        <Button
          startIcon={<AddCircleOutlineIcon />}
          onClick={() => addListItem("subRecipes")}
          sx={{ mt: 1 }}
        >
          添加{subRecipeTitle}
        </Button>
