...
const guideSections = [
  ...
  },
  {
    ...
    content: '库存总览页面不仅展示所有原料的合计库存，还提供了强大的库存快照功能，用于历史追溯和数据恢复。\n\n- **查看实时库存**: 默认视图显示所有原料在各岗位的实时库存总和及总价值。列表支持点击列标题排序。您可以点击每个原料的库存数量来展开查看其在各岗位的分布详情。\n- **导出Excel**: 点击页面右上角的"导出Excel"按钮，可以导出当前实时库存数据为Excel文件。导出内容包括原料名称、采购单位、规格、总库存以及各岗位的库存明细。\n- **查看历史快照**: 使用页面中部的下拉菜单，可以选择一个历史快照进行查看。快照按年份和周数显示，如"2025年 第3周"。在快照视图下，所有数据均为该快照创建时的数据。\n- **清理岗位库存**: 点击"清理岗位库存"按钮，系统会弹窗要求您确认。可以选择是否在生成快照后清空当前库存。确认后，系统会以当前各岗位的库存数量为准，生成一份全店的库存快照。此操作通常在每周盘点结束后进行。\n- **从快照还原库存**: 在下拉菜单中选择一个历史快照后，"从此快照还原库存"按钮会变为可用。点击并确认后，系统会将所有岗位的库存数量恢复到该快照记录的状态。这是一个高风险操作，请谨慎使用。\n- **快照对比**: 点击"对比快照/计算消耗"按钮，可以选择两个时间点（快照或当前库存）进行对比，计算期间的库存消耗情况。',
  },
  {
    id: 'inventory-check',
    title: '库存盘点 (InventoryCheckPage.jsx)',
    content: '库存盘点页面用于记录和更新各个岗位特定原料的实际库存数量。\n\n- **选择岗位**: 首先从下拉菜单中选择要盘点的操作岗位（如搅拌、丹麦、整形、烤炉等）。\n- **查看/更新库存**: 选择岗位后，会列出该岗位负责的所有原料。可以直接在"当前库存"列的输入框中修改数量。系统会显示当前已填写的库存总量。\n- **批量提交**: 修改库存后，点击"提交库存数据"按钮将所有数据一次性保存到服务器。如果有未填写的项目，系统会弹窗提醒并询问是否继续。\n- **数据来源**: 原料列表和其负责岗位信息来源于"物料管理"页面的配置。每个原料的post字段决定了它属于哪些岗位。',
  },
  {
    id: 'receiving',
    title: '收货入库 (ReceivingPage.jsx)',
    content: '收货入库页面用于记录原料的采购入库情况，通过扫描或手动录入更新库存。\n\n- **扫描入库**: 使用摄像头扫描原料包装上的文字信息，系统会自动识别原料名称和数量。\n- **手动录入**: 也可以直接选择原料、输入数量进行入库操作。\n- **库存更新**: 入库后会自动更新对应原料在"收货入库"岗位的库存数量。\n- **历史记录**: 可以查看历史的收货入库记录。',
  },
  {
