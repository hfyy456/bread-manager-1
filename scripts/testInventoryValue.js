...
const testInventoryValue = async () => {
  try {
    ...
    for (const ingredientData of testIngredients) {
      ...
      console.log(
        `  ✓ 创建原料: ${ingredient.name} - ¥${ingredient.price}/${ingredient.unit}`
      );
    }
    console.log("");
    ...
    for (const inventoryData of testInventories) {
      ...
      const ingredient = createdIngredients.find(
        (ing) => ing._id.toString() === inventoryData.ingredientId.toString()
      );

      // 计算预期价值
      const mainValue =
        (inventoryData.mainWarehouseStock?.quantity || 0) * ingredient.price;
      ...
      console.log(
        `    主仓: ${inventoryData.mainWarehouseStock?.quantity || 0}${
          ingredient.unit
        } = ¥${mainValue.toFixed(2)}`
      );
      console.log(
        `    岗位: ${Object.values(inventoryData.stockByPost || {}).reduce(
          (sum, stock) => sum + (stock.quantity || 0),
          0
        )}${ingredient.unit} = ¥${postValue.toFixed(2)}`
      );
    }
    ...
    } = require("../express/controllers/inventoryValueController");

    // 创建模拟的req和res对象
    const mockReq = {
      header: (name) =>
        name === "x-current-store-id" ? testStore._id.toString() : null,
    };
    ...
    if (apiResult && apiResult.success) {
      ...
      console.log("\n✅ 验证结果:");
      console.log(
        `  主仓价值差异: ¥${mainDiff.toFixed(2)} ${mainDiff < 0.01 ? "✓" : "✗"}`
      );
      console.log(
        `  岗位价值差异: ¥${postDiff.toFixed(2)} ${postDiff < 0.01 ? "✓" : "✗"}`
      );
      console.log(
        `  总价值差异: ¥${totalDiff.toFixed(2)} ${totalDiff < 0.01 ? "✓" : "✗"}`
      );
      ...
      console.log(
        `  有主仓库存: ${apiResult.data.counts.itemsWithMainStock}种`
      );
      console.log(
        `  有岗位库存: ${apiResult.data.counts.itemsWithPostStock}种`
      );
