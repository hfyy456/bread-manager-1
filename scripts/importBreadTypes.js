...
const BreadType = require('../express/models/BreadType');

const dataPath = path.join(__dirname, '..', 'src', 'data', 'breadTypes.json');
...
async function importData() {
  try {
    ...
    if (operations.length > 0) {
      const result = await BreadType.bulkWrite(operations);
